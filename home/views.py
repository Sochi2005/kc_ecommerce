...
def signin(request):
    if request.method == "POST":
        ...
        if user is not None:
            login(request, user)
            messages.success(request, f"welcome {username}! FlickBusters, we hope you have the best experience")
            return redirect("index")
        else:
...
def password(request):
    ...
    return render(request, "paswword.html", context)
...
def deleteitems(request):
    try:
        if request.method == "POST":
            ...
            deletecart.delete()
            messages.success(request, f"{items} has been deleted successfully...")
            return redirect("displaycart")
    except Shopcart.DoesNotExist:
        messages.error(request, f"{deletecart.title} Has been deleted successfully...")
        return redirect("displaycart")
...
def pay(request):
    if request.method == "POST":
        api_key = "sk_test_b4e98a1bf8308df6e17ab9353bc20953edfcf756"
        ...
        email = profile.email
        total = request.POST["total"] * 100
        phone = request.POST["phone"]
        address = request.POST["address"]
        headers = {"Authorization": f"Bearer {api_key}"}
        data = {"refrence": ref, "callback_url": cburl, "order_number": shop_code, "amount": int(total), "email": email, "currency": "NGN"}
